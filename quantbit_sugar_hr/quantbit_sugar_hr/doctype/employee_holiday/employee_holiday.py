...
class EmployeeHoliday(Document):
	...
	def add_staff(self):
		...
		employees = frappe.get_all("Employee",filters={"company":self.company, "branch":self.branch,"status":"Active"},fields=["name","employee_name","date_of_joining"])
	...
	def mark_attendance(self):
		...
		if public_holiday:
			...
			for holiday in public_holidays.holidays:
				if holiday.holiday_date <= to_date and holiday.holiday_date >= from_date:
					holiday_list.append(formatdate(holiday.holiday_date, "yyyy-MM-dd"))

		if self.season_type == "On Season":
			for holiday_date in self.holiday_date_details:
				if holiday_date.check and holiday_date.date not in holiday_list:
					holiday_list.append(formatdate(holiday_date.date, "yyyy-MM-dd"))
		...
		if self.season_type == "Off Season":
			...
			for i in range(total_days):
				...
				if day_name == off_days:
					holiday_list.append(formatdate(current_date, "yyyy-MM-dd"))
		...
		for employee in self.employee_holiday_details:
			for holiday_date in holiday_list:
				...
				if holiday_date and employee.joining_date:
					holiday_date = datetime.strptime(holiday_date, "%Y-%m-%d").date()
					if holiday_date >= employee.joining_date:
